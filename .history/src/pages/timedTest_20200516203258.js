  ...
  render() {
    ...
    return !this.state.result ? (
      <div className="center">
        ...
        <div className="center">
        <h1 ><strong >Total Questions: {data.length}</strong></h1>
        <button onClick={()=>this.setState({loadTimer:true})}>Start</button>
        </div>
        </React.Fragment>
        }
      </div>
          ...
          {data.map((res, index) => (
            <React.Fragment>
              {Number(this.state.answers[index]) === Number(res.a) ? (
                this.setState({score:this.setState.scote++}),
                <i class="fa fa-check fa-3" aria-hidden="true">
                  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                </i>
              ) : (
